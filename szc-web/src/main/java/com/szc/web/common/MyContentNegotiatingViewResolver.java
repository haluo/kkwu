...
public class MyContentNegotiatingViewResolver extends WebApplicationObjectSupport implements ViewResolver, Ordered  {
     ...
     * {@link #resolveViewName(String, java.util.Locale)} when an acceptable view cannot be found. This will allow for view
     * resolvers chaining. When this property is set to {@code true},
     * {@link #resolveViewName(String, java.util.Locale)} will respond with a view that sets the response status to
     ...
     * will fall back to using a {@link javax.activation.FileTypeMap} when the Java Action Framework is available.
     */
     ...
     * <p>The default implementation will check the {@linkplain #setMediaTypes(java.util.Map) media types} property first for a
     * defined mapping. If not present, and if the Java Activation Framework can be found on the class path, it will call
     * {@link javax.activation.FileTypeMap#getContentType(String)}
     * <p>This method can be overriden to provide a different algorithm.
     ...
     * <p>The default implementation will check the {@linkplain #setMediaTypes(java.util.Map) media types}
